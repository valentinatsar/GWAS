...
gemma -bfile boutsko_qc -maf 0.0001 -miss 0.2 -gk 1 -o Rmatrix   # -maf and -miss are used to avoid the default quality control of GEMMA, -gk defines the type of matrix (1:centered)

# 3. Run the GWAS for phenotype "daily grazing activity"
gemma -bfile boutsko_qc -n 1 -c covar-daily-activity.txt -maf 0.0001 -miss 0.21 -k Rmatrix.cXX.txt -lmm 4 -o GWAS_daily_activity  # -lmm - linear mixed model and "4" option - 3 statistical 
                                                                                                                                                          # approaches to estimate raw p-values - we use wald test p-val

# 4. Run the GWAS for phenotype "altitude difference" (optional step for practicing with a 2nd trait)
gemma -bfile boutsko_qc -n 1 -c covar-daily-activity.txt -maf 0.0001 -miss 0.21 -k Rmatrix.cXX.txt -lmm 4 -o GWAS_results-daily_activity

# The results file, called "GWAS_daily_activity.assoc.txt", includes one row for each SNP (all SNPs will be ordered by chromosome and base pair position) and the following columns:
	...
	- beta: Beta coefficient (SNP effect size on the trait studied) -> how much the presence of the allele changes the trait value
...
awk '{print $2,$1,$3,$13}' GWAS_daily_activity.assoc.txt > GWAS_results-daily_activity.txt
sed -e '1s/rs/SNP/' -e '1s/chr/CHR/' -e '1s/ps/BP/' -e '1s/p_wald/P/' GWAS_results-daily_activity.txt > GWAS_results-daily_activity_final.txt 
